...
for template in $(ls .vimpkg/template-save);
do
    rm .vimpkg/bundle/vim-template/templates/$template
    ln -s $CWD/.vimpkg/template-save/$template $CWD/.vimpkg/bundle/vim-template/templates/
done
